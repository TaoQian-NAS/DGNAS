...
class ChaoticParticleSwarm:
    ...
    def get_fitness(self, particle):  # 适应函数
        ...
        transformation_selection, propagation_selection, param_selection = self.hybrid_search_space.get_instance_pso(
            particle)
        reset_pso_parameters(self.args, transformation_selection, propagation_selection, param_selection)
        print('============================\n skip={}, Hid={}, act={}, nlayer={}, norm={}, hops={}, agg={}, dropout={}, lr={}, weight_decay={}'.format(
            self.args.skip_connections, self.args.dim_hidden, self.args.activation_function, self.args.num_layers,
            self.args.graph_normalization, self.args.propagation_layers, self.args.aggregator_type, self.args.dropout,
        self.args.lr, self.args.weight_decay))
        ...
        features = aggregator(self.args.aggregator_type, target).detach_()
        # features = y.div_(self.args.propagation_layers + 1.0).detach_()
        ...
        trainset = features[self.idx_train]
        trainlabel = labels[self.idx_train]
        t_total = time.time()
        best_val = 0
        best_test = 0
...
#             optimizer.step()
            if self.args.batch_size == 0:
                ...
                for i in range(int(self.idx_train.shape[0]/self.args.batch_size)):
                    optimizer.zero_grad()
                    curtrain = trainset[i*self.args.batch_size:(i+1)*self.args.batch_size]
                    curlabel = trainlabel[i*self.args.batch_size:(i+1)*self.args.batch_size]
                    output = model(curtrain)
                    loss_train = F.nll_loss(output, curlabel)
                    acc_train = accuracy(output, curlabel)
